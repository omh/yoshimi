...
class TestPolicy:
    ...
    def test_can_select_when_location_is_not_self(self):
        c1 = Content()
        c1.id = 5
        assert self.policy.can_select(c1) is True

    def test_can_not_select_location_when_location_is_self(self):
        c1 = Content()
        c1.id = 2
        assert self.policy.can_select(c1) is False
    ...
    def test_can_select_when_not_child_of_self(self):
        ...
        c2.id = 4
        assert self.policy.can_select(c2) is True
    ...
    def test_can_not_select_when_child_of_self(self):
        ...
        c2 = Content(parent=c1)
        c2.id = 5
        assert self.policy.can_select(c2) is False
